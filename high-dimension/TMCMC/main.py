...
ParameterInfo = {
    ...
    "ParameterName": [f"k{i}" for i in range(1, 21)],
...
UpdateParameterName = [f"k{i}" for i in range(1, 21)]
...
measure_vector = np.arange(20).reshape(1, 20)
...
prior_mean = np.full((1, 20), 2)
i = 0
all_pars = [pdfs.Normal(mu=prior_mean[0, i] * TrueUpdateParameterValues[i],
                                 sig=0.1 * np.abs(prior_mean[0, i] * TrueUpdateParameterValues[i]))
            for i in range(DOF)]
